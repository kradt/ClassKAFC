...
def remove_lesson():
	...
	try:
		data = cabinet_service.remove_user_lesson(db=db.session, user_uuid=flask_login.current_user.uuid, lesson_name=lesson)
	except ValueError:
...
def update_user():
	...
	try:
		cabinet_service.update_user(db=db.session, user_uuid=flask_login.current_user.uuid, lesson_name=lesson, name=name)
	except DataError:
...
def send_task():
	...
	if form.validate_on_submit():
		...
		try:
			validate_task = TaskCreate(title=form.title.data, description=form.description.data, group=form.group.data, lesson=lesson)
			db_task = cabinet_service.create_task(db=db.session, task=validate_task, user_uuid=flask_login.current_user.uuid, file=form.file.data)
